...
export default function explorer(query: QueryHandler) {
  ...
  const getTransactionByTokenAddressOfAddress = async (
    tokenAddress: string,
    address: string,
    opts: Options = {
      date: new Date(),
    ...
    const rowSql = `
      ...
      FROM
        bigquery-public-data.goog_blockchain_ethereum_mainnet_us.token_transfers
      WHERE
        (LOWER(from_address) = LOWER('${address}') OR
        LOWER(to_address) = LOWER('${address}')) AND
        LOWER(address) = LOWER('${tokenAddress}') AND
        DATE(block_timestamp) = DATE('${d}')
      ORDER BY block_timestamp DESC
      LIMIT ${limit}
      OFFSET ${offset}
    `;
  ...
  return {
    ...
    getTransactionByTokenAddressOfAddress,
  };
}
